...
final safeProjectsProvider = Provider<List<Project>>((ref) {
  ...
  print('[SAFE PROJECTS] hasValue=${async.hasValue} isLoading=${async.isLoading} returning=${result.length}');
  return result;
});
...
final safeClientsProvider = Provider<List<Client>>((ref) {
  ...
  final result = async.valueOrNull ?? [];
  print('[SAFE CLIENTS] hasValue=${async.hasValue} isLoading=${async.isLoading} returning=${result.length}');
  return result;
});
...
class RecentActivityNotifier extends AsyncNotifier<List<Activity>> {
  ...
  Future<List<Activity>> build() async {
    ...
    if (authState.status != AuthStatus.authenticated) {
      print('[ACTIVITY BUILD] not authenticated - clearing cache');
      _lastActivities = [];
      return [];
    }

    final uid = authState.user?.id ?? SupabaseService.userId;
    print('[ACTIVITY BUILD] uid=$uid cacheLen=${_lastActivities.length}');
    ...
    // );
    ref.onDispose(() {
      print('[ACTIVITY DISPOSED]');
      _subscription?.cancel();
    });

    if (_lastActivities.isNotEmpty) {
      print('[ACTIVITY BUILD] returning CACHED ${_lastActivities.length} activities');
    ...
    try {
      ...
      print('[ACTIVITY BUILD] initial fetch: ${result.length} activities');
      _lastActivities = result;
      return result;
    } catch (e) {
      print('[ACTIVITY BUILD] fetch failed: $e');
      return [];
    }
  }
}
...
final dashboardMetricsProvider = Provider<DashboardMetrics>((ref) {
  final projects = ref.watch(safeProjectsProvider);
  final clients = ref.watch(safeClientsProvider);
  print('[DASHBOARD METRICS] projects=${projects.length} clients=${clients.length}');
